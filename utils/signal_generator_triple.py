...
class TripleConfirmSignalGenerator:
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 初始化信號
        df['signal'] = 0
        df['signal_strength'] = 0.0  # 信號強度 (0-1)
        ...
        # 計算信號強度 (根據RSI偏離程度和MACD強度)
        df.loc[long_conditions, 'signal_strength'] = (
            # RSI越低,信號越強
            (self.rsi_oversold - df.loc[long_conditions, 'rsi']) / self.rsi_oversold * 0.5 +
            # MACD正值越大,信號越強
            np.clip(df.loc[long_conditions, 'macd_diff'] / df.loc[long_conditions, 'close'] * 100, 0, 0.5)
        )
        
        df.loc[short_conditions, 'signal_strength'] = (
            # RSI越高,信號越強
            (df.loc[short_conditions, 'rsi'] - self.rsi_overbought) / (100 - self.rsi_overbought) * 0.5 +
            # MACD負值越大,信號越強
            np.clip(-df.loc[short_conditions, 'macd_diff'] / df.loc[short_conditions, 'close'] * 100, 0, 0.5)
        )
    ...
    def get_signal_description(self, row: pd.Series) -> str:
        """獲取信號描述"""
        if row['signal'] == 1:
            return f"LONG: Price={row['close']:.2f}, RSI={row['rsi']:.1f}, MACD={row['macd']:.4f}, ADX={row['adx']:.1f}"
        elif row['signal'] == -1:
            return f"SHORT: Price={row['close']:.2f}, RSI={row['rsi']:.1f}, MACD={row['macd']:.4f}, ADX={row['adx']:.1f}"
        else:
            return "NO SIGNAL"
    ...
    def get_signal_summary(self, df: pd.DataFrame) -> Dict:
        ...
        return {
            ...
            'signal_frequency': len(signals) / len(df) * 100,  # 信號頻率(%)
        }
...
if __name__ == '__main__':
    ...
    # 生成信號
    generator = TripleConfirmSignalGenerator()
