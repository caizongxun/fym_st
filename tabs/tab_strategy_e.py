"""
策略E: Candle Pattern ML v3
主要改進:
1. 信號冷卻 (cooldown): 避免连續進場
2. 趨勢過濾: EMA排列才進場
3. 高信心度門溻: 預設0.62
4. 目標盈虧比>=2.0
"""
...
class CandlePatternML:
    ...
    def generate_signals(self, df, conf_long=0.62, conf_short=0.62,
                         position_pct=0.7, stop_atr_mult=1.5, tp_atr_mult=3.0,
                         cooldown_bars=3, use_trend_filter=True):
        """
        信號生成 v3:
        - cooldown_bars: 進場後最少隔幾根K棒再進
        - use_trend_filter: EMA8>20>50才做多; <才做空
        """
        X, df_aligned = self.build_sequence_features(df)
        ...
        signals = []
        last_signal_bar = -cooldown_bars  # 初始化
        ...
        for i in range(len(df_aligned)):
            ...
            sig = 0; sl = np.nan; tp = np.nan; reason = ""

            # 冷卻期檢查
            in_cooldown = (i - last_signal_bar) < cooldown_bars

            if not in_cooldown:
                long_ok  = pl >= conf_long  and pl > ps
                short_ok = ps >= conf_short and ps > pl
...
def render_strategy_e_tab(loader, symbol_selector):
    st.header("策略 E: K棒影線 AI v3")

    st.info("""
    **v3 改進重點**:
    - 冷卻期 (cooldown): 進場後必須等N根K棒才可再進
    - EMA趨勢確認: EMA8>20>50才做多 | EMA8<20<50才做空
    - 預設信心度提高至0.62 (減少信號數量)
    - 目標盈虧比 3.0 (TP=3xATR vs SL=1.5xATR)
    - 增加 min_samples_leaf 防止過擬合
    """)
    ...
    with col3:
        ...
        threshold_atr = st.slider("訊號門溻(ATR)", 0.2, 1.5, 0.4, 0.1, key="thr_e")
        conf_long     = st.slider("做多信心度", 0.40, 0.90, 0.62, 0.01, key="cl_e")
        conf_short    = st.slider("做空信心度", 0.40, 0.90, 0.62, 0.01, key="cs_e")
        cooldown      = st.slider("冷卻期(K棒數)", 1, 20, 5, key="cd_e")
        trend_filter  = st.checkbox("開啟EMA趨勢過濾", value=True, key="tf_e")
        st.markdown("**風控**")
        stop_atr = st.slider("止損ATR", 0.5, 3.0, 1.5, 0.5, key="sl_e")
        tp_atr   = st.slider("止盈ATR", 1.5, 6.0, 3.0, 0.5, key="tp_e")
    ...
    if st.button("訓練 + 回測", type="primary", use_container_width=True):
        ...
        try:
            ...
            if isinstance(loader, BinanceDataLoader):
                ...
                df_all = loader.load_historical_data(
                    symbol, '15m', end - timedelta(days=total_days), end)
            else:
            ...
            df_for_bt = pd.concat([
                df_test.iloc[:lookback].reset_index(drop=True), df_aligned
            ...
            st.markdown("---")
            if wr >= 50 and pf >= 1.5:
                st.success("✅ 有效!"); st.balloons()
            elif ret > 0:
                st.info("⚠️ 有獲利 - 繼續優化")
            else:
                # 具體關鍵發現
                if wr < 45:
                    st.warning("❌ 勝率不足 - 提高信心度 or 增加訓練天數")
                elif pf < 1.2:
                    st.warning("❌ 盈虧比不足 - 提高TP或降低SL")
                else:
                    st.warning("❌ 改調冷卻期或趨勢過濾")
            ...
            if not trades.empty:
                ...
                st.dataframe(
                    trades[['entry_time','direction','entry_price',
                             'exit_price','pnl_usdt','exit_reason']].tail(30),
                    use_container_width=True
                )
                csv = trades.to_csv(index=False).encode('utf-8')
                st.download_button("CSV", csv,
                    f"{symbol}_ml_{datetime.now():%Y%m%d_%H%M}.csv", "text/csv")
