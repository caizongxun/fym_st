...
def render_backtest_tab(loader):
    ...
    with col5:
        stop_loss_pct = st.number_input("止損 (%)", 0.5, 5.0, 2.0, 0.5)
    
    with col6:
        take_profit_pct = st.number_input("止盈 (%)", 1.0, 10.0, 3.0, 0.5)
...
def run_backtest(loader, symbol, test_days, model_package,
    ...
    with st.spinner("正在回測..."):
        try:
            ...
            # 準備資料
            df = prepare_backtest_data(df, model_package['params'])
            ...
            if row['touch_upper'] and upper_prob >= prob_threshold:
                ...
                if entry_price is not None:
                    position = open_position(
                        ...
                        capital, position_size_pct, fee_rate, slippage,
                        upper_prob
                    )
...
def check_exit(position, row, stop_loss_pct, take_profit_pct):
    entry_price = position['entry_price']
    direction = position['direction']
