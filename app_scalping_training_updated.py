...
with tabs[3]:
    ...
    **模型選擇**:
    ...
    st.caption(f"訓練: {scalp_train_candles}根 | OOS: {scalp_oos_days}天 | 模型: {model_type.upper()} | 目標: {target_pct*100:.1f}% | 預測: {lookforward}根K線 | 風報比: {risk_reward}")
        ...
        with st.spinner(f"正在訓練 {scalp_train_symbol} 剝頭皮模型 (OOS模式)..."):
            try:
                ...
                if isinstance(loader, BinanceDataLoader):
                    ...
                    df_full = loader.load_historical_data(scalp_train_symbol, '15m', start_date, end_date)
                else:
                ...
                # 訓練模型 (OOS模式)
                trainer = ScalpingModelTrainer(model_dir='models/saved', model_type=model_type)
                ...
                # 保存模型
                trainer.save_model(scalp_train_symbol, prefix=f'scalping_{model_type}_oos')
                st.success(f"模型已保存: models/saved/{scalp_train_symbol}_scalping_{model_type}_oos_scalping_{model_type}.pkl")
                
                # 評估模型
                if metrics['oos_accuracy'] >= 0.60 and val_oos_gap < 0.10:
                    st.balloons()
                    st.success("OOS準確率 ≥ 60% 且泛化良好! 可以進行回測")
                elif metrics['oos_accuracy'] >= 0.55:
                    st.info("模型表現尚可，建議試試不同模型或調整參數")
                else:
                    st.warning(f"OOS準確率偏低: {metrics['oos_accuracy']:.2%}，建議:")
                    st.write("1. 增加訓練數據")
